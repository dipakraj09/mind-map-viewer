...
let gridOn=false, mmOn=false, panelOn=false, xhOn=false, focusOn=false;
let curTheme='';
...
function toggleFocus() {
  ...
  // auto-close panels in focus mode
  if (focusOn) {
    if (panelOn) togglePanel();
  }
  toast(focusOn ? '🌟 Focus Mode ON — Sab hide ho gaye!' : '🌟 Focus Mode OFF — Sab wapas aa gaye!');
}
...
document.addEventListener('keydown', e => {
  ...
  switch (e.key) {
    ...
    case 'Escape':
      if (document.getElementById('settingsOv').classList.contains('open'))   toggleSettings();
